...
```{r}
# btw if you do command return on the line of a function, you get just that one function executed. **Note: maybe ask later about how below work together to produce these results
head(primary_24)
summary(primary_24)
colnames(primary_24)
glimpse(primary_24)
...
```{r}
primary_24 |>
  # select() says these are columns I want keep, ditch the rest of the columns
  select(office_name, office_district, candidate_name, party, county_name, election_day, votes) |>
  #^ if we were to save that to a data set, would see fewer variables (8) in the environment tab at right
  # create new column without grouping by something as did in lab 1 when used summarize(), bc (reminder) summarize() would make new data set when grouping by a column value-- group_by and summarize always stay together, that's why
  mutate(
    # the equal sign means what's after it is what goes in the new column
...
Made a new column, called percent_election_day, which is the proportion of votes that were cast on election day (for the corresponding office, office district, candidate, party, and county).
...
Now it's by descending value for new column made
...
primary_24 |>
  ...
  # the default (when not "desc") is ascending
  # for some reason, it won't run anything if you type "asc"
...
Possibly; Unlike the last results, this immediately shows us that in Garrett County, Prince George's County, and Talbot County, there were no election day votes. The counties listed thereafter had a small percent that were election day. For a story that includes talking about the decrease in the number of people voting on election day (as has been happening over the last several years), this is quite relevant. (There would also be comparison between counties in the story; not only generally stating a trend w time.)
...
```{r}
# this is a way of cleaning the data a little to make it more clear and usable!
# create a new variable named standardized_maryland_cities that holds the data from maryland_cities ...
standardized_maryland_cities <- maryland_cities |>
  # ... with a new column
  mutate(
    # named upper_city, which has values from the city column but in uppercase instead
    # new function called string to upper
    upper_city = str_to_upper(city)
)
# can also do str_to_lower() if wanted to, btw
```
...
Note: "String" refers to text, a text value, sometimes similar or interchangeable with character like character data type 

Note: There are lots of potential uses for standardization - addresses, zip codes, anything that can be misspelled or abbreviated.
...
**ANSWER:**
Use mutate to create a new column called amount_category. Specify what each row will say (for the variable amount_category) for five monetary domains/ranges/intervals, using case_when within the mutate function.

```{r}
# putting stuff in new variable
maryland_winred_categories <- maryland_winred_contributions |>
  # use mutate to create a new column...
  mutate(
    #...called amount_category, and what goes in that is...
    amount_category = case_when(
      # when the value in the amount column is less than 100, make the value in our new amount_category column "Less than $100"
      # we don't know why "~" operator is for assigning, but it is
...
# then can sort by amount descending and see yes "3,000 or more" at top, etc.
...
prince_georges <- primary_24 |>
  # filter to only the rows where county_name is EXACTLY "Prince George's County"
...
Because now it's easy to retrieve the data table that shows only these columns for PG County.
...
```{r}
# the following is same result as code above in last codebox
...
or_prince_georges <- primary_24 |>
  # give the rows where the county name is Prince George's County OR where the office name is U.S. Senator
  #"or" is additive-> you'll get more results, versus "and" which gives fewer bc something has to be this and that and so on (if multiple this and that)
  filter(county_name == "Prince George's County" | office_name == "U.S. Senator")
...
# note from looking at full data set, you're prob not seeing a full set of all the data for this that exists. Just something to be aware of when you're looking at data in general, including things from online and FOIA
...
maryland_expenses <- maryland_expenses |>
  # mdy in function mdy() means month-date-year. It refers to the dates' current format, not the format we are converting them to. We are converting them to what I'll think of as ymd, which R properly reads as dates
mutate(expenditure_date=mdy(expenditure_date))

head(maryland_expenses)
```

**ANSWER:** The dates now appear as year-month-day, and R treats them as dates.
...
maryland_expenses |>
  ...
  # floor_date function with "month" means the first of the month (the month to which a given date belongs)
  # the first time we type month below, it assigns the name of the new column
  # the second time we type month below, in quotes, this means group by the month and the first day (day as a sub-unit of a month)
  # If you change what's in quotes to a different date or time element, floor_date() will group by that and so order with the "floor" as the first of the corresponding sub-unit. For example, if you type floor_date(expenditure_date, "year"), total_amount is by year with the first of January for every year (bc month is a sub-unit of year). could also do "day" 
