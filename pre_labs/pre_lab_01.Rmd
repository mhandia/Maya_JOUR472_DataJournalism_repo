...
This notebook contains code and explanatory text that you should review and run as you read through chapters 1 - 5 of the course textbook, "Data Journalism with R and the Tidyverse". Answer questions and edit the document as directed.
...
**Task** Run the following code in the gray-colored codeblock below -- not in the console -- to load the tidyverse library. To run the code, click the little green play button at the top right of the codeblock. In Rmarkdown data notebooks, we write code inside of codeblocks, and explanatory text in the white area outside of it.

```{r}
# This is a comment inside of a codeblock. Comments don't run when we run the codeblock.
# Adding this is a good way to start your class reference notebook.
library(tidyverse)
```
...
469 obs. (rows or records) 7 variables (columns or fields)
...
"last_name" is a chr column / data type. Probably represents the last name of a person who gave a donation to a candidate. "amount" is a dbl column / data type. "amount" might be the amount of money someone donated to a candidate.
...
```{r}
# head() function shows first six rows of data
# there are other numbers of rows you can show w different function(s) fyi
# but head() is by default six rows
...
```{r}
# same name of data set, then tell R going to do something with/to it using pipe operator |>
maryland_winred_contributions |>
  # group together all identical values from date column, then do something else with data
  group_by(date) |>
  # let's do something with those groupings
  summarise(
    # create new column titled count_contribs containing values that each equal the number of rows (number of donations) that have that same date value
    count_contribs = n()
  )
```
...
created new column titled count_contribs containing values that each equal the number of rows (number of donations) that have that same date
...
maryland_winred_contributions |>
  ...
  # arranging the data in descending order by the values in count_contribs
...
Arranging the data in descending order of the values in count_contribs (number of contributions). We are now seeing it organized this way instead of by ascending date values.
...
The data is now grouped by memo_text (recipient name) and amount (of money for a given contribution), still giving the number of contributions per grouping. (But now grouping by two things.) Ex: 5915 people each gave exactly $21.07 to TRUMP NATIONAL COMMITTEE JFC, INC. Notice there is repetition of recipient name (memo_text) because there are several amounts of money for that recipient. 
...
The data is now only grouped by recipient name (memo_text), each with the number of contributions and the total amount of money received. Data is arranged in descending order of total amounts of money received (per recipient).
...
The summary table now includes the mean and median amount of money in a donation, for each recipient. Arranged in descending order of the number of contributions.
...
Data now grouped by city instead of recipient; summary includes min and max donation amount (for a given city). Arranged by descending total contribution amount (for each city).

### Final note

Remember how we saved a single number, and then a data set, inside variables that we named? Well, we can also do that with the results of the analysis we were doing above.

Let's take all the code we just ran, and store the results inside the new variable maryland_contribs_summarized

```{r}
maryland_contribs_summarized <- maryland_winred_contributions |>
  group_by(city) |>
  summarise(
    count_contribs = n(),
    total_amount = sum(amount),
...
Unlike last time, we don't see the results of our code right away when we save the output in a variable.

What happens when we just run the name of the variable, though?

```{r}
maryland_contribs_summarized
```

(Answer: We see the results of the code!)
